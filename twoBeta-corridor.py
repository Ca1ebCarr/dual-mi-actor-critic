...
def plot_state_similarity_matrix_spacial(
    encoder, env, beta_1, beta_2, rep_dim, save_dir="./outputs/corridor/similarity"
):
    """
...
if __name__ == "__main__":
    ...
    GAMMA = 0.99
    MAX_STEPS = 30  # must match KeyDoorEnv(max_steps=...)
    REP_DIM = 8
    ...
    for BETA_1, BETA_2 in BETA_VALUES:
        ...
        sim_matrix, sim_labels = plot_state_similarity_matrix_spacial(
            ...
            beta_2=BETA_2,
            rep_dim=REP_DIM,
            save_dir="./outputs/corridor/similarity/spacial",
        )
        ...
        sim_matrix, sim_labels = plot_state_similarity_matrix_directional(
            ...
            beta_2=BETA_2,
            rep_dim=REP_DIM,
            save_dir="./outputs/corridor/similarity/directional",
        )
    ...
    with open("./outputs/corridor/numerical_results.json", "w") as f:
    ...
    goal_keys_found = sorted({k for r in runs for k in r["goal_fractions"]})
    goal_frac = {
        k: np.array([r["goal_fractions"].get(k, 0.0) for r in runs])
        for k in goal_keys_found
    }
    ...
    # Shared helpers
    goal_labels = {
        "1_1": "R=1  (zone A, top-left)",
        "9_1": "R=2  (zone B, top-right)",
        "1_7": "R=4  (zone C, bottom-left)",
        "9_7": "R=6  (zone D, bottom-right)",
    }
    goal_colors = {
        "1_1": "#e74c3c",
        "9_1": "#f39c12",
    ...
    plt.tight_layout()
    plt.savefig("./outputs/corridor/fig1_reward_vs_beta.png", dpi=150, bbox_inches="tight")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig("./outputs/corridor/fig2_3d_reward.png", dpi=150, bbox_inches="tight")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig("./outputs/corridor/fig3_heatmaps.png", dpi=150, bbox_inches="tight")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig("./outputs/corridor/fig4_mi_quantities.png", dpi=150, bbox_inches="tight")
    plt.close()
    ...
    fig, axes = plt.subplots(
        2, len(goal_keys_found), figsize=(5 * len(goal_keys_found), 9)
    )

    fig.suptitle(
        "Goal Visit Rate — ZonedCorridorEnv",
        fontsize=13,
        fontweight="bold",
    )

    for col, gk in enumerate(sorted(goal_keys_found)):
        gf = goal_frac[gk] * 100
        label = goal_labels.get(gk, gk)
    ...
    plt.tight_layout()
    plt.savefig("./outputs/corridor/fig5_goal_distribution.png", dpi=150, bbox_inches="tight")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(
        "./outputs/corridor/fig6_latent_compression.png", dpi=150, bbox_inches="tight"
    )
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig("./outputs/corridor/fig7_success_rate.png", dpi=150, bbox_inches="tight")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig("./outputs/corridor/fig8_reward_steps.png", dpi=150, bbox_inches="tight")
    plt.close()
    print("Saved fig8_reward_steps.png")

    print("\nAll figures saved to ./outputs/corridor/")
